...
- **Strict `redirect_uri` allowlist** — operator-configured `broker.oauth.allowed_redirect_uris` (HTTPS-only; defaults to claude.ai's two callbacks but extensible to any remote MCP client). No loopback, no wildcard. Enforced server-side at DCR and at `/oauth/authorize` POST. With no identity layer at `/oauth/authorize`, the allowlist + PKCE + the client's own `state` validation IS the security boundary.
