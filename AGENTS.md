...
- Outbound OAuth credentials (broker-as-client tokens for upstream providers) MUST be encrypted at rest via `MultiFernet`, not plain `Fernet`. **Rationale:** key rotation is required — `MultiFernet` tries each key in order, so an operator can add a new key today and remove the old one later without downtime. Inbound bearer + refresh tokens (broker-as-AS) are SHA-256-hashed instead, matching the api-key pattern — they are validate-only and never decrypted for replay, so hashing-without-decrypt is sufficient.
...
- Outbound OAuth state (broker-as-client) MUST be HMAC-signed with single-use nonces and a ≤10-minute TTL. **Rationale:** unsigned state enables CSRF; replayable state enables login-fixation. Short TTL bounds the replay window. Inbound `state` (broker-as-AS) belongs to the remote MCP client and is echoed back unchanged — the AS does not own it.
...
- Outbound OAuth discovery (broker resolving upstream MCP servers) MUST reject private, loopback, and link-local addresses. **Rationale:** an attacker-controlled upstream could redirect discovery to `169.254.169.254` (cloud metadata) or internal services. SSRF defence.
...
- Inbound refresh-token rotation MUST be atomic under `BEGIN IMMEDIATE` with a conditional `UPDATE used_at WHERE used_at IS NULL`; replay (`rowcount=0` with the row still present and `used_at` already set) MUST cascade-delete the entire token family. **Rationale:** OAuth 2.1 §4.3.1 mandates rotation. Without atomicity, two concurrent refresh requests race and both mint a new family, defeating replay detection.
- Inbound `redirect_uri` MUST be on the operator-configured `broker.oauth.allowed_redirect_uris` allowlist (HTTPS-only), enforced both at DCR (registration time) and at `/oauth/authorize` POST (consent time). The consent page CSP MUST NOT set `form-action` — server-side enforcement is the boundary, and CSP3 would otherwise block the legitimate 302 to the client callback. **Rationale:** with no identity layer at `/oauth/authorize`, the allowlist + PKCE + the client's own state validation IS the security boundary. Loosening to "any HTTPS" would let an attacker DCR with their own callback and exfiltrate authorization codes. Loopback / arbitrary-HTTP support is a v1.5 item gated behind explicit config.
- Inbound DCR `X-Forwarded-For` MUST be trusted only when `request.client.host` is in `broker.oauth.trusted_proxy_ips`. **Rationale:** unconditional XFF trust lets a direct-access attacker bypass the per-IP rate limit by cycling the header. Default-empty allowlist means XFF is ignored entirely for direct-access deployments.
- Inbound OAuth on a non-Firestore store backend MUST run on a single uvicorn worker. **Rationale:** with the default (sqlite / in-memory) backend the DCR rate limiter and the outbound nonce/PKCE state are process-local; N workers would mean the effective limit is N × the configured cap and per-flow state could land on a different worker. Startup aborts if `broker.oauth.enabled=true` AND `WEB_CONCURRENCY>1` AND `store.backend != "firestore"`. With `store.backend: firestore` all inbound auth state, connect tokens, outbound nonce/PKCE, and the DCR limiter are shared across instances, so multi-worker / multi-replica is legal and the abort does not fire.
...
2. **Cascade deletion** — Deleting a broker key cascades to outbound OAuth tokens via `TokenStore.delete_all_for_app` AND to inbound OAuth state via `SQLiteInboundAuthStore.delete_all_for_app` (auth codes + bearer/refresh tokens; DCR clients are not app-bound and are kept). If you add new per-app persisted state elsewhere, the cascade MUST cover it too. **Rationale:** re-provisioning the same `app_key` after a compromise silently regains access to any leaked-then-retained state.
...
4. **In-memory state** — the connect-token store, the outbound nonce/PKCE state store, and the inbound DCR rate limiter each have an in-memory implementation (the default) AND a Firestore-backed implementation (`FirestoreConnectTokenStore`, `FirestoreOutboundOAuthStateStore`, `FirestoreDCRRateLimiter`). The lifespan selects the Firestore versions when `store.backend: firestore`; otherwise the in-memory defaults are used and remain process-local. Any NEW in-memory state added to middleware or services MUST be called out — and, if it gates multi-instance behaviour, given a Firestore-backed implementation behind its ABC. **Rationale:** horizontal scaling requires moving state to a shared backing. Single-instance assumptions leak into code as `dict` or `set` without the author realising. With the in-memory defaults the DCR rate limiter is the reason inbound OAuth aborts at startup when `WEB_CONCURRENCY>1`; with the Firestore backend that state is shared and the abort is lifted.
...
| **Managed-key** | `auth_mode='managed_key'` (native or remote) | Broker injects a per-app **static** key read from `apps.{client}.{app}.{connector}.api_key` as the credential — native handlers receive it as `access_token`, remote forwards inject `Authorization: Bearer`. No OAuth flow (`requires_oauth=False`, `uses_managed_key=True`). |
...
**No-OAuth connectors (`auth_mode='none'`).** A connector whose upstream is an open API — or one it authenticates to with its own static credential — sets `auth_mode='none'`. The broker then skips the OAuth connection gate entirely (`resolve_oauth` + `get_valid_token`): no `/oauth/{name}/connect` flow, no stored token, no `upstream_not_connected`. A native `auth_mode='none'` connector's handlers receive an **empty** `access_token` and empty `provider_metadata`; the connector self-sources any static credential from its own broker-side config (e.g. an env var it reads directly). No OAuth URLs and no `apps` credentials are required for these connectors. **Rationale:** the broker is OAuth-centric, but internal/first-party APIs are often open on a private network or gated by a single static token — forcing a fake OAuth flow on them would be a hack; `auth_mode='none'` is the sanctioned bypass.

**Managed-key connectors (`auth_mode='managed_key'`).** For an API-key service (e.g. image generation) where each app brings its **own** static key but there is no OAuth flow to populate the token store. Unlike `auth_mode='none'` (empty token; the connector self-sources one shared credential), `managed_key` makes the broker read a **per-app** key from `apps.{client}.{app}.{connector}.api_key` and inject it as the handler's `access_token` (native) or as `Authorization: Bearer` (remote forward) — the same per-app delivery path as OAuth tokens, sourced from config instead of an OAuth exchange. `requires_oauth=False` (no `/oauth/{name}/connect`); a request for an app with no `api_key` returns `503 managed_key_not_configured`. The key is a secret: never logged, and a non-native `managed_key` connector's `mcp_url` MUST be HTTPS (the `_validate_url_schemes` HTTPS guard covers both `broker` and `managed_key`). Like static-connector credentials, the `api_key` value MUST be referenced via `${VAR}` interpolation in `settings.yaml` (`api_key: ${MY_CONNECTOR_API_KEY}`) — the raw key lives in `.env`, never committed in YAML. `mcp_oauth_url` MUST NOT be set on a `managed_key` connector (it is a no-OAuth mode; a discovery URL would mislead the connect wizard — enforced by `_validate_url_schemes`). `uses_managed_key` computes the flavour. **Rationale:** per-tenant paid APIs (image gen, etc.) need a per-app key but have no OAuth — config-sourced per-app injection gives per-tenant isolation without faking an OAuth flow.
