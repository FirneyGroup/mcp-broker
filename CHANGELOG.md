...
- **`auth_mode='managed_key'` connectors** — for an API-key service (e.g. image generation) where each app brings its **own** static key but there is no OAuth flow to populate the token store. The broker reads the per-app key from `apps.{client}.{app}.{connector}.api_key` and injects it as the native handler's `access_token` — the same per-app delivery path used for OAuth tokens, sourced from config instead of an OAuth exchange. `requires_oauth` is `False` (no `/oauth/connect`); a missing per-app `api_key` returns `503 managed_key_not_configured`. New `ConnectorMeta.uses_managed_key` property. Existing connectors are unaffected.
