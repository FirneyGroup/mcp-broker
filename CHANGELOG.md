...
- **File URLs on `notion_api` block reads** — `fetch`, `get_block_children`, `append_blocks`, and the block-content tools now surface a `url` field on file-bearing blocks (`image`, `file`, `pdf`, `video`, `audio`), resolving both Notion-hosted files (`file.url`) and external ones (`external.url`). Previously the block reductions dropped the URL, so an asset attached to a page could not be fetched back by a reader. The Notion-hosted URL is signed and short-lived (~1h), so callers must read fresh at use time; a not-yet-resolved upload carries no `url`. Additive — non-file blocks and existing fields are unchanged.
