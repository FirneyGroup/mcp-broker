...
def normalize_resource(raw: str) -> str:
    ...
    Raises ValueError on a fragment (RFC 8707 forbids fragments in resource indicators).
    Queries are tolerated and ignored per RFC 8707 §2 (SHOULD NOT include, but we
    accept for robustness).
    """
    parsed = urlparse(raw)
    if parsed.fragment:
